...
func (h *Handlers) createAccount(w http.ResponseWriter, r *http.Request) {
    type req struct{ Owner, Currency string `json:"owner"`; InitialBalance int64 `json:"initial_balance"` }
    var body req
...
func (h *Handlers) enqueueTx(w http.ResponseWriter, r *http.Request) {
    type req struct{ AccountID string `json:"account_id"`; Type string `json:"type"`; Amount int64 `json:"amount"`; IdempotencyKey string `json:"idempotency_key"` }
    var body req
    if err := json.NewDecoder(r.Body).Decode(&body); err != nil { http.Error(w, err.Error(), 400); return }
    key := body.IdempotencyKey
    if key == "" { key = r.Header.Get("Idempotency-Key") }
    if key == "" { key = uuid.NewString() }
    msg := queue.TxMessage{ AccountID: body.AccountID, Type: body.Type, Amount: body.Amount, Key: key, CreatedAt: time.Now() }
    if err := h.Pub.Publish(r.Context(), msg); err != nil { http.Error(w, err.Error(), 500); return }
...
func (h *Handlers) enqueueTransfer(w http.ResponseWriter, r *http.Request) {
    type req struct{ FromAccountID string `json:"from_account_id"`; ToAccountID string `json:"to_account_id"`; Amount int64 `json:"amount"`; IdempotencyKey string `json:"idempotency_key"` }
    var body req
    if err := json.NewDecoder(r.Body).Decode(&body); err != nil { http.Error(w, err.Error(), 400); return }
    key := body.IdempotencyKey
    if key == "" { key = r.Header.Get("Idempotency-Key") }
    ...
    if err := h.Pub.PublishTransfer(r.Context(), msg); err != nil { http.Error(w, err.Error(), 500); return }
    w.WriteHeader(http.StatusAccepted)
    json.NewEncoder(w).Encode(map[string]string{"status":"queued","idempotency_key":key})
}
